    ...
    _createClass(AgendaResourceEvents, [{
        ...
        value: function render() {
            ...
            var startDate = schedulerData.startDate,
                ...
                userId =  schedulerData.config.userId,
                bgColorExceptLoggedinUser =  schedulerData.config.bgColorExceptLoggedinUser;

            var agendaResourceTableWidth = schedulerData.getResourceTableWidth();
            // var width = agendaResourceTableWidth - 2;
            var width = schedulerData.config.agendaResourceTableWidth;
            ...
            return _react2.default.createElement(
                'tr',
                { style: { height: config.eventItemLineHeight + 2 } },
                _react2.default.createElement(
                    'td',
                    {className: "text-titlize", 'data-resource-id': resourceEvents.slotId },
                    slotItem
                ),
                _react2.default.createElement(
                    // 'td',
                    // { style: { verticalAlign: "baseline" }, onClick: function onClick() {
                    //     newEvent(schedulerData, resourceEvents.slotId, resourceEvents.slotName, schedulerData.startDate, schedulerData.endDate);
                    // } },
                    'td',
                    { style: bgColorExceptLoggedinUser && userId != resourceEvents.slotId ? { verticalAlign: "baseline", backgroundColor: config.nonWorkingTimeBodyBgColor }: { verticalAlign: "baseline"} },
                    _react2.default.createElement(
                        'div',
