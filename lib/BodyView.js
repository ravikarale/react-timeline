    ...
    _createClass(BodyView, [{
        ...
        value: function render() {
            ...
            var tableRows = displayRenderData.map(function (item) {
                ...
                console.log(bgColorExceptLoggedinUser, item.slotId, userId)
                return _react2.default.createElement(
                    'tr',
                    { key: item.slotId, style: bgColorExceptLoggedinUser && item.slotId != userId ? { height: item.rowHeight, backgroundColor: config.nonWorkingTimeBodyBgColor } : { height: item.rowHeight } },
                    rowCells
                );
            });
