        ...
        value: function render() {
            ...
            if (cellUnit === _index.CellUnits.Hour) {
                ...
                headerList = headers.map(function (item, index) {
                    ...
                    if(viewType === 1) {
                        var pFormattedList = config.nonAgendaWeekCellHeaderFormat.split('|').map(function (item) {
                            var dateArr = datetime.format(item).split(' ')
                            return  dateArr.slice(0, 1).join(" ").toUpperCase() +" | " + dateArr.slice(1).join(" ")
                        });
                    }else if(viewType === 2) {
                        var pFormattedList = config.nonAgendaMonthCellHeaderFormat.split('|').map(function (item) {
                            return datetime.format(item);
                        });
                    }else{
